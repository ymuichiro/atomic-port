...
(async () => {
  ...
  const client = new HTLCERC20Service(Contracts.rinkeby.erc20.endpoint, Contracts.rinkeby.erc20.contractAddress);
  ...
  // mint
  const result = await client.mint(toAddress, fromAddress, hashPair.secret, 1, TOKEN.JPYC);
  ...
  console.log('withDraw', `${res.result.transactionHash}`);
