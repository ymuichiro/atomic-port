...
async function withDraw(contractId: string, proof: string) {
  ...
  const client = new EvmErc20Htlc(NETWORK.TEST.sepolia.native.endpoint, NETWORK.TEST.sepolia.native.contractAddress);
  ...
  console.log(
    `----- Start withDraws https://mumbai.polygonscan.com/tx/${res.result.transactionHash} -----`,
