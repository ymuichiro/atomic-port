...
const { htlcERC20ArrayToObj, newSecretHashPair, nowSeconds, txContractId } = require('./helper/utils');
...
contract('HashedTimelock swap between two ERC20 tokens', (accounts) => {
  ...
  it('Step 1: Alice sets up a swap with Bob in the AliceERC20 contract', async () => {
    ...
    const newSwapTx = await newSwap(
      AliceERC20,
      htlc,
      { hashlock: hashPair.hash, timelock: timeLockSeconds },
      Alice,
      Bob
    );
