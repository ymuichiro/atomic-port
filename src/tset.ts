...
async function erc20Test(){
  ...
  const draw = await _etherErc20Htlc.withDraw(contractId, _etherErc20Htlc.web3.eth.accounts.wallet[1].address, (mint as any).hashPair.secret, GAS);
...
async function erc721Test(tokenId: number){
  ...
  const draw = await _etherErc721Htlc.withDraw(contractId, _etherErc721Htlc.web3.eth.accounts.wallet[1].address, (mint as any).hashPair.secret, GAS);
...
async function createErc721(tokenId: number){
  const erc721TokenContract = new _etherHtlc.web3.eth.Contract(ERC721Abi.abi as any, "0xA0Ce6944AcCa20a9201E36a42C6a2a4c7e42aB72");
